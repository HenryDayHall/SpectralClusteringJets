...
{\color{magenta} in the algorithms proposed}; all references to rapidity $y$ correspond to
...
\subsection{Spectral clustering algorithm}\label{sec:spectralmethodalgo}
    ...
    \item \label{step:affinity} The affinity must increase as pseudojets become more similar,
        ...
        {\color{magenta}where $\alpha=2$ is the standard Gaussian kernel as
...
\item\label{step:laplacean} These affinities allow the construction of the symmetric normalised
    ...
    \item \label{step:eigenvectors} The eigenvectors of $L(t)$, ($q$ the eigenvalue index)
            ...
            {NOTE: \color{magenta} Eigenvalues $0\leq \lambda(1)\leq 2$.
        ...
        \item\label{step:stoppingcondition}
            ...
            {\color{magenta} NOTE: there are $\frac{c(c-1)}{2}$ possible pairs, 
            ...
            \sout{The two pseudojets that have been combined are removed and replaced with one pseudojet.
...
\begin{itemize}
    ...
    \item  \(\alpha\): also introduced in step~\ref{step:affinity},
       ...
       \item  \(\lambda_\text{limit}\): introduced in step~\ref{step:eigenvectors}, is a means of limiting the number of eigenvectors used
           ...
           {\color{magenta} and $\lambda_k$ is related to the quality of forming $k$
       ...
       \item  \(\beta\): introduced in step~\ref{step:compression}, it 
        ...
        {\color{magenta} corresponding to higher eigenvalues}
        \sout{with lower quality information} are compressed.
        \sout{The larger the value of \(\beta\) the more dimensions with
...
%
    ...
    and \(\lambda_\text{limit}\) are relatively {\color{magenta} unconstrained},
